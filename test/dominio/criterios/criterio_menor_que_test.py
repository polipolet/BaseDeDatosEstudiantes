...
class CriterioMenorQueTest(SuperClassForTests):
    ...
    def __init__(self,*args, **kwargs):
        ...
        CriterioMenorQueTest.numInstances = CriterioMenorQueTest.numInstances + 1
        if CriterioMenorQueTest.numInstances == 1:
            CriterioMenorQueTest.numErrorsBefore = len(SuperClassForTests.indErrors)


    def setUpClass():
        # SuperClassForTests.nota = {}
        CriterioMenorQueTest.nota = 0.0
        print("\n\nCORRIGIENDO clase CriterioMenorQue")
        print("**************************")

    def tearDownClass():
        SuperClassForTests.showErrors(SuperClassForTests.indErrors, "CriterioMenorQue")
        SuperClassForTests.acumulaErrores()
        SuperClassForTests.indErrors = []
        CriterioMenorQueTest.nota = SuperClassForTests.puntosTotales
    ...
    def test01__init__(self):
        ...
        try:
            print(f"\n\tCriterioMenorQue::__init__(nombre_clave,valor_a_comprobar). Valor: {valorTotal}" )
            individual_per = valorTotal/num_tests
            nombre_clave = "nombreClave-1"
            valor_a_comparar = "20"
            instance = CriterioMenorQue(nombre_clave,valor_a_comparar)
    ...
    def test02_se_cumple(self):
        ...
        try:
            ...
            instance = CriterioMenorQue(nombre_clave,valor_a_comparar)
            print("\tTest 1: comprobación cuando la fila de datos tiene mapeado con la clave un valor (20.0) menor que "
            ...
            #
            print("\tTest 2: comprobación cuando la fila de datos tiene mapeado con la clave un valor (20.0) mayor que "
            ...
            error = self.sAssertFalse(instance.se_cumple(fila),valorTotal/num_tests,"Error. La fila de datos tenía "
                + "mapeado con la clave un valor mayor que valor a comprobar, pero el método ha devuelto True")
            self.toThrow(error,toThrow)
            #
